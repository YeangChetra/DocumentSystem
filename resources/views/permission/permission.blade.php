<x-app-layout>
  <main>
    <div class="container-fluid">
      <h2 class="mt-30 page-title">Permissions</h2>
      <ol class="breadcrumb mb-30">
        <li class="breadcrumb-item"><a href="{{url('/')}}">Dashboard</a></li>
        @canany('User access','User add','User edit','User delete')
          <li class="breadcrumb-item"><a href="{{route('users.index')}}">User Accounts</a></li>
        @endcanany
        <li class="breadcrumb-item active h-title">Permissions</li>
      </ol>
      <form class="formApply" id="formApply" method="post" action="{{ route('permissions.destroy', 100) }}">
        {{csrf_field()}}
        @method('delete')
        <div class="row justify-content-between">
          @can('Permission create')
          <div class="col-lg-12" style="text-align:right;">
            <a href="{{route('permissions.create')}}" class="btn btn-primary" data-toggle="modal" data-target="#addModal">Add
              New</a>
          </div>
          @endcan
            ...
            <div class="card card-static-2 mb-30">
              ...
              <div class="card-body-table">
                <div class="table-responsive">
                  <table class="table ucp-table table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td><input type="checkbox" class="form-control check-item" name="ids[]" value="{{$permission->id}}">
                        </td>
                        <td>{{$permission->name}}</td>                        
                        <td>{{$permission->guard_name}}</td>
                        <td>{{ \Carbon\Carbon::parse($permission->create_date)->format('d-M-y h:i A')}}</td>
                        <td class="action-btns">
                          @can('Permission edit')
                            <a href="{{route('permissions.edit',Crypt::encrypt($permission->id))}}" class="edit-btn" title="Edit"
                              data-toggle="modal" data-target="#editModal"><i class="fas fa-edit edit"
                                id="{{Crypt::encrypt($permission->id)}}">Edit</i></a>
                          @endcan
                        </td>
                      </tr>
                      @endforeach
                ...
                <div class="text-center" style="float: left;padding-top: 15px;padding-left: 5px;">
                  <p style="font-size: 14px;">Showing {{$permissions->firstItem()}} to {{$permissions->lastItem()}} of
                    {{$permissions->total()}} entries</p>
                </div>
                <div class="text-center" style="float: right;padding-top: 5px;padding-right: 5px;">
                  <nav>
                    @if ($permissions->lastPage() > 1)
                    <ul class="pagination">
                      <li class="page-item {{ ($permissions->currentPage() == 1) ? ' disabled' : '' }}" aria-disabled="true"
                        aria-label="« Previous">
                      ...
                      @for ($i = 1; $i <= $permissions->lastPage(); $i++)
                        <?php
                          ...
                          $from = $permissions->currentPage() - $half_total_links;
                          $to = $permissions->currentPage() + $half_total_links;
                          if ($permissions->currentPage() < $half_total_links) {
                            $to += $half_total_links - $permissions->currentPage();
                          }
                          if ($permissions->lastPage() - $permissions->currentPage() < $half_total_links) {
                              $from -= $half_total_links - ($permissions->lastPage() - $permissions->currentPage()) - 1;
                          }
                        ?>
                        @if ($from < $i && $i < $to) <li
                          class="page-item{{ ($permissions->currentPage() == $i) ? ' active' : '' }}" aria-current="page">
                          ...
                          <li class="page-item{{ ($permissions->currentPage() == $permissions->lastPage()) ? ' disabled' : '' }}">
                            <a class="page-link" href="{{ $permissions->url($permissions->lastPage()) }}" rel="next"
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <form class="formPermission" id="formPermission" method="post" action="{{route('permissions.store')}}">
            @csrf
            @method('post')
            <div class="modal-body">
              <div class="form-group">
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}"
                  class="form-control @error('name') is-invalid @enderror" placeholder="Enter Permission"
              ...
              <div class="form-group mb-3">
                <label class="form-label tl-color">permissions</label>
              </div>
              <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                  ...
                  <?php
                    $permission_lists = Config::get('globalvariable.permissions');
                  ?>
                  @foreach($permission_lists as $k => $v)
                    <div class="col-lg-3 col-md-3 col-sm-3 col-3">
                      <div class="form-group mb-3">
                        <div class="custom-control custom-checkbox">
                          <input class="cb-element custom-control-input" id="{{$v}}" name="permissions_list[]"
    ...
    <!--//modal edit-->
    <form class="formPermissionUpdate" id="formPermissionUpdate" method="post" action="{{ Session::has('urlUpdatePermission') ? Session::get('urlUpdatePermission') :'' }}">
      @csrf
      @method('put')
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="form-group">
                <label class="form-label">Permission *</label>
                <input type="text" id="name" name="name" value="{{ old('name') }}"
                  class="form-control @error('name') is-invalid @enderror" placeholder="Enter Permissions"
                  data-error=".errorTxt1">
                <div class="errorTxt1 msg-error"></div>
                  @if(\Session::has('urlUpdatePermission'))
                    @error('name')
